...
int menu()
{
    ...
    cout << "[4] Lista de veiculos por placa " << endl;
    cout << "[5] Registro completo dos veiculos " << endl;
    cout << "[6] Trocar veiculos no registro " << endl;
    cout << "[0] Sair " << endl;

    // Laco para exibicao do menu dentro dos valores possiveis
    while (!(cin >> choice) || choice < 0 || choice > 6) // Enquanto a negação do input choice em cin for verdadeira OU choice < 1 OU choice > 7
    {
...
{   
    ...
    int qt = 0, size = 50, c = 0, qt_suv = 0, qtauto = 0, qtflex = 0, qt_el_pwr = 0, i = 0, z = 0, qt_model = 0, var_assist = 0, from = 0, to = 0, aux = 49;
    float sum_prices = 0, avg_prices = 0, pricesuv = 0, avg_prices_suv = 0, perc_auto = 0, perc_el_pwr = 0;
        // Declaracao do vetor
    ...
    // Abertura do arquivo
    ifstream read ("BD_Veiculos2..txt");
    ...
    while (next != "FIM")
    {
       // Limpando a tela no inicio da execucao
       system("cls");
    ...
    avg_prices_suv = (pricesuv/qt_suv); // Media de precos dos SUVs
    perc_auto = (((float)qtauto/qt)*100); // Porcentagem de veiculos automaticos
    perc_el_pwr = (((float)qt_el_pwr/qt)*100); // Porcentagem de veiculos eletricos com motor 1.8
    ...
    while((choice = menu())!=0)
    {
        ...
        switch (choice)
        {
            ...
            case 4:
                // Saida de dados
                cout << "--------------------------------------------" << endl;
                cout << "Lista de veiculos por placa: " << endl;
                cout << "--------------------------------------------" << endl;
                for (i = 0; i < qt; i++)
                {
                    cout << i << ": " << arraycar[i].license << endl;
            ...
            case 6:
                // Entrada de dados
                while ((0 < from < 32) && (0 < to < 32)){
                    cout << "Informe o número da linha a ser trocada: " << endl;
                    cin >> from;
                    cout << "Informe o número da linha destino: " << endl;
                    cin >> to;
                }

                // Troca no registro
                arraycar[aux] = arraycar[from];
                arraycar[from] = arraycar[to]; 
                arraycar[to] = arraycar[aux]; 
